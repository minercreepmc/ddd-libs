import { DateVO, ID, UUID } from '@domain/value-objects';
import { ArgumentInvalidException } from 'ts-common-exceptions';
import { DomainEvent, DomainEventOptions } from './domain-event.abstract';

describe('DomainEvent', () => {
  describe('isValidEventOptions', () => {
    it('should throw an exception if eventOptions is empty', () => {
      const eventOptions: DomainEventOptions<unknown> = {} as any;
      expect(() => {
        DomainEvent.isValidEventOptions(eventOptions);
      }).toThrow(ArgumentInvalidException);
    });

    it('should not throw an exception if eventOptions is not empty', () => {
      const eventOptions: DomainEventOptions<unknown> = {
        entityId: new ID('123'),
        entityType: 'TestEntity',
        eventName: 'TestEvent',
        details: { foo: 'bar' },
      };
      expect(() => {
        DomainEvent.isValidEventOptions(eventOptions);
      }).not.toThrow();
    });
  });
  it('should set eventId to provided eventId or generate a new UUID', () => {
    const eventId = new ID('123');
    const domainEventOptions: DomainEventOptions<any> = {
      eventId,
      entityId: new ID('456'),
      entityType: 'test',
      eventName: 'testEvent',
      details: {},
    };

    const domainEvent = new DomainEvent(domainEventOptions);
    expect(domainEvent.eventId).toBe(eventId);

    const domainEventOptions2: DomainEventOptions<any> = {
      entityId: new ID('456'),
      entityType: 'test',
      eventName: 'testEvent',
      details: {},
    };
    const domainEvent2 = new DomainEvent(domainEventOptions2);

    expect(UUID.isValid(domainEvent2.eventId)).toBe(true);
  });
  it('should set dateOccurred to the provided dateOccurred or the current date', () => {
    const dateOccurred = DateVO.create(new Date(2020, 1, 1));
    const domainEventOptions: DomainEventOptions<any> = {
      eventId: new ID('123'),
      entityId: new ID('456'),
      entityType: 'test',
      eventName: 'testEvent',
      dateOccurred,
      details: {},
    };
    const domainEvent = new DomainEvent(domainEventOptions);
    expect(domainEvent.dateOccurred).toBe(dateOccurred);

    const domainEventOptions2: DomainEventOptions<any> = {
      eventId: new ID('123'),
      entityId: new ID('456'),
      entityType: 'test',
      eventName: 'testEvent',
      details: {},
    };
    const domainEvent2 = new DomainEvent(domainEventOptions2);
    expect(domainEvent2.dateOccurred.equals(DateVO.now())).toBe(true);
  });
  it('should set entityId, entityType, eventName, and details based on provided eventOptions', () => {
    const entityId = new ID('456');
    const entityType = 'test';
    const eventName = 'testEvent';
    const details = {};
    const domainEventOptions: DomainEventOptions<any> = {
      eventId: new ID('123'),
      entityId,
      entityType,
      eventName,
      details,
    };
    const domainEvent = new DomainEvent(domainEventOptions);
    expect(domainEvent.entityId).toBe(entityId);
    expect(domainEvent.entityType).toBe(entityType);
    expect(domainEvent.eventName).toBe(eventName);
    expect(domainEvent.details).toBe(details);
  });
});
